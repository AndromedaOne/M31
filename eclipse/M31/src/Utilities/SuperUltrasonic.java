...
/**
 * SuperUltrasonic rangefinder class. The SuperUltrasonic rangefinder measures absolute distance based on the
 * round-trip time of a ping generated by the controller. These sensors use two transducers, a
 * speaker and a microphone both tuned to the SuperUltrasonic range. A common SuperUltrasonic sensor, the
...
public class SuperUltrasonic extends SensorBase implements PIDSource, LiveWindowSendable {
  ...
  // head of the SuperUltrasonic sensor list
  private static SuperUltrasonic m_firstSensor = null;
  ...
  private SuperUltrasonic m_nextSensor = null;
  ...
  // The time delay for ping because thats the given time for a round
  // trip if our max distance is four feet away from whatever we use them on. 
  // Calculator proven. Bill approved.
  private double m_timeDelayPing = .007;

  /**
   * Background task that goes through the list of SuperUltrasonic sensors and pings each one in turn.
   ...
   * none of the SuperUltrasonic sensors will change while it's running. If one does, then this will
  ...
  private class UltrasonicChecker extends Thread {
    ...
    public synchronized void run() {
      SuperUltrasonic ultrasonic = null;
      while (m_automaticEnabled) {
        ...
        Timer.delay(m_timeDelayPing); // wait for ping to return
      }
    }
  }
  
  public void setTimeDelayPing(double delayTime){
	  m_timeDelayPing = delayTime;
  }
  
  /**
   * Initialize the SuperUltrasonic Sensor. This is the common code that initializes the SuperUltrasonic
  ...
  /**
   * Create an instance of the SuperUltrasonic Sensor. This is designed to supchannel the Daventech SRF04
   * and Vex SuperUltrasonic sensors.
   *
   ...
   */
  public SuperUltrasonic(final int pingChannel, final int echoChannel, Unit units) {
  ...
  /**
   * Create an instance of the SuperUltrasonic Sensor. This is designed to supchannel the Daventech SRF04
   * and Vex SuperUltrasonic sensors. Default unit is inches.
   *
   * @param pingChannel The digital output channel that sends the pulse to initiate the sensor
   *                    sending the ping.
   * @param echoChannel The digital input channel that receives the echo. The length of time that
   ...
   *                    distance.
   */
  public SuperUltrasonic(final int pingChannel, final int echoChannel) {
    this(pingChannel, echoChannel, Unit.kInches);
  }

  /**
   * Create an instance of an SuperUltrasonic Sensor from a DigitalInput for the echo channel and a
   ...
   * @param units       The units returned in either kInches or kMilliMeters
   */
  public SuperUltrasonic(DigitalOutput pingChannel, DigitalInput echoChannel, Unit units) {
   ...
   *                    range.
   */
  public SuperUltrasonic(DigitalOutput pingChannel, DigitalInput echoChannel) {
    this(pingChannel, echoChannel, Unit.kInches);
  }

  /**
   * Destructor for the SuperUltrasonic sensor. Delete the instance of the SuperUltrasonic sensor by freeing
  ...
  public synchronized void free() {
    ...
    if (this == m_firstSensor) {
      ...
      for (SuperUltrasonic s = m_firstSensor; s != null; s = s.m_nextSensor) {
   ...
   *
   * @param enabling Set to true if round robin scheduling should start for all the SuperUltrasonic
  ...
  public void setAutomaticMode(boolean enabling) {
    ...
    if (enabling) {
       ...
       */
      for (SuperUltrasonic u = m_firstSensor; u != null; u = u.m_nextSensor) {
        u.m_counter.reset();
      }
  ...
  /**
   * Single ping to SuperUltrasonic sensor. Send out a single ping to the SuperUltrasonic sensor. This only
  ...
  /**
   * Get the range in inches from the SuperUltrasonic sensor. If there is no valid value yet, i.e. at
   * least one measurement hasn't completed, then return 0.
   *
   * @return double Range in inches of the target returned from the SuperUltrasonic sensor.
   */
  ...
  /**
   * Get the range in millimeters from the SuperUltrasonic sensor. If there is no valid value yet, i.e.
   * at least one measurement hasn't completed, then return 0.
   *
   * @return double Range in millimeters of the target returned by the SuperUltrasonic sensor.
   */
  ...
  public void setPIDSourceType(PIDSourceType pidSource) {
    if (!pidSource.equals(PIDSourceType.kDisplacement)) {
      throw new IllegalArgumentException("Only displacement PID is allowed for SuperUltrasonics.");
    }
  ...
  /**
   * Is the SuperUltrasonic enabled.
   *
   * @return true if the SuperUltrasonic is enabled
   */
  ...
  /**
   * Set if the SuperUltrasonic is enabled.
   *
   * @param enable set to true to enable the SuperUltrasonic
   */
